...
namespace Win32 {
	...
	namespace Utils {
		...
		BOOL PAGE_API AddBitmap(CONST WCHAR* szFileName, HDC hWinDC, INT x = 0, INT y = 0);

        /// Sets and clears style flags for a particular window.
        inline VOID PAGE_API ModifyWindowStyle(HWND hWnd, DWORD flagsToDisable, DWORD flagsToEnable)
        {
        ...
        inline VOID PAGE_API ModifyWindowExStyle(HWND hWnd, DWORD flagsToDisable, DWORD flagsToEnable)
        {
        ...
        inline BOOL PAGE_API HasStyle(HWND hwnd, DWORD style) {
        ...
        /// Sets and clears style flags for a particular window.
        inline VOID PAGE_API ModifyClassStyle(HWND hWnd, DWORD flagsToDisable, DWORD flagsToEnable)
        {
            DWORD style = GetWindowLong(hWnd, GCL_STYLE);
            SetClassLong(hWnd, GCL_STYLE, (style & ~flagsToDisable) | flagsToEnable);
        }

        inline BOOL PAGE_API IsWindowFullscreen(HWND hWnd)
        {
        ...
        inline VOID PAGE_API MaximizeWindow(HWND hwnd) {
